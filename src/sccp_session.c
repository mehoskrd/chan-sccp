...
static void *accept_thread(void *ignore)
{
	...
	for (;;) {
		...
		if ((proto = getprotobyname("tcp"))) {
			...
			sccp_netsock_setoptions(new_socket, /*reuse*/ 0, /*linger*/ -1, /* keepalive */ -1);
		}
...
void sccp_session_start_accept_thread(void)
{
...
boolean_t sccp_session_bind_and_listen(struct sockaddr_storage *bindaddr)
{
	...
	if (accept_sock < 0) {
		int status;
		int port = sccp_netsock_getPort(bindaddr);
		...
		do {
			accept_sock = socket(res->ai_family, res->ai_socktype, res->ai_protocol);			// need to add code to handle multiple interfaces (multi homed server) -> multiple socket descriptors
			if (accept_sock < 0) {
			...
			sccp_log((DEBUGCAT_CORE)) (VERBOSE_PREFIX_3 "SCCP: started listening on %s:%d\n", addrStr, port);
